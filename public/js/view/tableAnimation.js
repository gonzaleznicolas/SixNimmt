...
class TableAnimation extends Animation
{
	...
	moveRows(fromRow, toRow, downThisManyRows, callback, callbackParam)
	{
		...
		// original value
		this._line = new CardMovementLine(this._drawer.Canvas.width, 0, 0, 0, downThisManyRows*(this._drawer.CardHeight + lc.margin));
		if (!this._line.done)
		{
	...
	sortUpcomingCards(callback, callbackParam)
	{
		...
		for (let i = 0; i < newPosition.length; i++)
		{
			...
			this._resourcesForCardOriginallyAtPositionI[i] = {
				...
				line : new CardMovementLine(this._drawer.Canvas.width, start.x, start.y, end.x, end.y),
				nextPt : null
			};
	...
	moveIthUpcomingCardToRowCol(i, tableRow, tableCol, callback, callbackParam)
	{
		...
		this._line = new CardMovementLine(this._drawer.Canvas.width, startXY.x, startXY.y, endXY.x, endXY.y);
		this._nextPt = null;
		if (!this._line.done)
	...
	takeRow(rowIndex, bDisapearAtTheEnd, callback, callbackParam)
	{
		...
		this._line = new CardMovementLine(this._drawer.Canvas.width, start.x, start.y, end.x, end.y);
		if (!this._line.done)
